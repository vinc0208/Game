	...
	if ((*shoot == (1 << 5)) && (ship->bullets > 0)){
		...
		*shoot = 0;
		...
		if ((ship->bullets <= 0) && (*reload_timer <= 0)) {
			*reload_timer = time_s + 60*time_m;
		}
	}
	if (((time_s + 60*time_m) == *reload_timer + 3) && (ship->bullets <= 0)){
...
void updateAsteroid(asteroid* ast, int n_ast){
	...
	for(i=0;i<n_ast;i++){
		if(ast[i].status!=0){ //check each active asteroid
			if((CheckOnScr(ast[i].x-1,ast[i].y)==0) || (CheckOnScr(ast[i].x,ast[i].y+1)==0)){ //check if they are near an edge
				ast[i].status=0;
